...
export default function WorkshopView({ shelves, boxes, tools, materials, searchQuery, onSearchChange, searchResults, onAddShelf, onAddBox, onAddTool, onAddMaterial, onUpdateShelf, onDeleteShelf, }) {
    ...
    const [initialShelf, setInitialShelf] = useState(null);
    ...
    return (_jsxs("div", { className: "flex flex-col h-full", children: [_jsx(WorkshopHeader, { onAddShelf: handleAddShelf, onAddBox: handleAddBox, onAddTool: handleAddTool, onAddMaterial: handleAddMaterial }), _jsxs("div", { className: "flex items-center gap-2 px-4 py-3 bg-orange-50 border-b border-orange-200", children: [_jsx("input", { value: searchQuery, onChange: (e) => onSearchChange(e.target.value), placeholder: "Suchen\u2026", className: "flex-1 px-3 py-2 border border-gray-300 rounded-md text-base focus:outline-none focus:ring-2 focus:ring-orange-400" }), _jsx("button", { onClick: () => setShowScanner(true), className: "w-10 h-10 flex items-center justify-center bg-orange-500 text-white rounded-md hover:bg-orange-600 transition", children: _jsx(QrCode, { className: "w-6 h-6" }) })] }), _jsxs("div", { className: "p-4", children: [_jsx(GlobalSearch, { shelves: shelves, boxes: boxes, tools: tools, materials: materials, query: searchQuery }), sortedShelves.map((shelf) => (_jsxs("div", { onClick: () => navigate(`/shelf/${shelf.id}`), className: "flex items-center p-3 border border-gray-300 rounded-lg mb-2 bg-white cursor-pointer hover:bg-gray-50 transition", children: [_jsx("div", { className: "flex-1 font-semibold", children: shelf.name }), _jsx("button", { onClick: (e) => {
                                    e.stopPropagation();
                                ...
                                }, className: "bg-red-500 text-white rounded-md p-2", children: _jsx(Trash2, { size: 18 }) })] }, shelf.id))), showScanner && (_jsx(QRScanner, { onScan: handleScan, onClose: () => setShowScanner(false) }))] }), showShelfForm && (_jsx(ShelfForm, { initialShelf: initialShelf ?? undefined, onSave: (shelf) => {
                    if (initialShelf) {
                        onUpdateShelf({
                            ...shelf,
                    ...
                    setShowShelfForm(false);
                    setInitialShelf(null);
                } })), showBoxForm && (_jsx(BoxForm, { shelves: shelves, initialBox: initialBox ?? undefined, onSave: (box) => {
                    if (initialBox) {
                        // EDIT
